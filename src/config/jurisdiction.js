...
export const SOURCE_REGISTRY = [
  ...
  {
    "id": "nyc_311_electric_hazards_rockaway",
    "name": "NYC 311 reported electric hazards and road blockages — Rockaway",
    "owner": "NYC 311",
    "geographies": [
      "rockaway"
    ],
    ...
    "refresh_seconds": 300,
    "stale_after_seconds": 900,
    "required_filter": "borough = 'QUEENS' AND community_board = '14 QUEENS' AND latitude IS NOT NULL AND longitude IS NOT NULL AND ((agency = 'HPD' AND complaint_type IN ('ELECTRIC','Electric') AND descriptor IN ('POWER OUTAGE','Power Outage')) OR (agency = 'DOB' AND complaint_type = 'Electrical' AND descriptor IN ('Electrical Wiring Defective','Electrical Wiring Exposed')) OR (agency = 'DPR' AND descriptor IN ('Hitting Power Lines','Hitting Power/Phone Lines','Hitting Phone/Cable Lines')) OR complaint_type IN ('Blocking Street','Blocked Road') OR (complaint_type = 'Street Condition' AND descriptor LIKE 'Blocked -%'))",
    "query_select": "unique_key,created_date,agency,complaint_type,descriptor,status,borough,community_board,incident_zip,latitude,longitude",
    "query_order": "created_date DESC",
    "query_limit": 500,
    "aggregate_queries": [
      {
        "key": "all_reported_hazards",
        "label": "All included reports",
        "query_select": "count(*) as total",
        "required_filter": "borough = 'QUEENS' AND community_board = '14 QUEENS' AND ((agency = 'HPD' AND complaint_type IN ('ELECTRIC','Electric') AND descriptor IN ('POWER OUTAGE','Power Outage')) OR (agency = 'DOB' AND complaint_type = 'Electrical' AND descriptor IN ('Electrical Wiring Defective','Electrical Wiring Exposed')) OR (agency = 'DPR' AND descriptor IN ('Hitting Power Lines','Hitting Power/Phone Lines','Hitting Phone/Cable Lines')) OR complaint_type IN ('Blocking Street','Blocked Road') OR (complaint_type = 'Street Condition' AND descriptor LIKE 'Blocked -%'))"
      },
      {
        "key": "direct_electric",
        "label": "Direct electrical reports",
        "query_select": "count(*) as total",
        "required_filter": "borough = 'QUEENS' AND community_board = '14 QUEENS' AND ((agency = 'HPD' AND complaint_type IN ('ELECTRIC','Electric') AND descriptor IN ('POWER OUTAGE','Power Outage')) OR (agency = 'DOB' AND complaint_type = 'Electrical' AND descriptor IN ('Electrical Wiring Defective','Electrical Wiring Exposed')))"
      },
      {
        "key": "electric_tree_or_wire",
        "label": "Tree or wire reports",
        "query_select": "count(*) as total",
        "required_filter": "borough = 'QUEENS' AND community_board = '14 QUEENS' AND agency = 'DPR' AND descriptor IN ('Hitting Power Lines','Hitting Power/Phone Lines','Hitting Phone/Cable Lines')"
      },
      {
        "key": "road_blockage",
        "label": "Road blockage reports",
        "query_select": "count(*) as total",
        "required_filter": "borough = 'QUEENS' AND community_board = '14 QUEENS' AND (complaint_type IN ('Blocking Street','Blocked Road') OR (complaint_type = 'Street Condition' AND descriptor LIKE 'Blocked -%'))"
      }
    ],
    "attribution": "NYC Open Data / NYC 311",
    "disclaimer": "NYC 311 records are reported complaints, not utility-confirmed outages or verified restoration constraints.",
    "operational_note": "Includes direct electrical reports, tree/wire reports, and any scoped blocked-road or blocked-street-condition reports because access constraints may affect restoration work.",
    "display": {
      "icon": "⚡",
      "color": "#facc15",
      "kind": "operational",
    ...
    "normalization": {
      ...
      "scope": {
        ...
        "rules": [
          {
            "key": "direct_electric",
            "label": "Direct electrical report",
            "all": [
              {
                "field": "agency",
                "values": [
