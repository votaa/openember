...
| `nyc_311_electric_hazards_rockaway` | NYC 311, dataset `erm2-nwe9` via NYC Open Data/Socrata | Rockaway only after filtering | 500-record map browse plus separate uncapped `count(*)` queries; app token recommended | implemented in Phase 4 | preserve explicit electrical/tree-wire/road-blockage rules; never present reports as utility-confirmed outages |
